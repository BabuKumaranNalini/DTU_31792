...
def create_model(ems_local):
    ...
    # Declare variables
    m.g_heat_cap, m.e_heat_cap, m.elec_import, m.elec_export = (pyen.Var(m.t, within=pyen.NonNegativeReals) for i in range(4))
    m.costs = pyen.Var(m.t, within=pyen.Reals)
    
    
    def elec_balance_rule(m, t):
        return m.elec_import[t] + m.solar[t]*m.pv_peak_power - m.elec_export[t] - m.elec_load[t] - m.e_heat_cap[t] == 0
...
def extract_res(m, ems):
    ...
    e_heat, g_heat, pv_pv2demand, pv_pv2grid, ele_heater, \
    gas_heater = (np.zeros(length) for i in range(11))
