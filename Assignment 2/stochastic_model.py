...
def stochastic_opt(g_data,Fmax,Bmax,Scen,Dem,phi):
    ...
    model.Pg_da = pyen.Var(model.g, within=pyen.NonNegativeReals)
    ...
    def Pg_DA_bound(model, g):
       return model.Pg_da[g] <= model.pg_max[g]
    model.pg_da_bound = pyen.Constraint(model.g, rule=Pg_DA_bound)
    ...
    def obj_rule(model):
        ...
        cost_net = 0.0
        for s in model.s:
            cost_rt += phi[s]*(sum(model.cg[g]*model.Pg_rt[g,s] for g in model.g) + \
                       sum(model.penalty[d]*model.L_shed[d,s] for d in model.d)) 
        cost_net = cost_rt + sum(model.cg[g]*model.Pg_da[g] for g in model.g)
        return cost_net
